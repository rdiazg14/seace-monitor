...
**Arquitectura C:** C1, C2, **C3** y **C4 implementados**. Defensa anti-drift = artefacto + consenso ×3 + ledger + cola admin. Gemini **no** está en el pipeline diario (ya dura ~49 min); solo semanal.
...
Encender `--gc` cuando se acepte borrar chunks de culminados. OCR más amplio si hay cuota. Reabrir 2 h / rpm 6 si la cola imagen no baja. No meter Gemini en el pipeline diario.
...
| Data lake | **2 061 PDFs / 2,34 GB**; `tdr/{YYYY}/{MM}/{id}/{aid}.pdf` | **MEDIDA** 10 sep | histórico anterior a los vigentes actuales | ~45 meses a 100 GB |
| Capas datos | **fases 0–6 aplicadas** (2026-09-10). `contratos` sin `categoria_it`/`relevancia_ia`; clasificación solo en `clasificacion_contrato`; eco eliminado; `v_contratos` 41 cols `security_invoker=true` | **MEDIDA** 10 sep (run 34437994154 verde) | migrar `auditoria-clasificador/` (no prod) | expand-contract **cerrado**; vuelta atrás = snapshot 4 593 filas |
| Verificar capas | `scripts/verificar_capas.py` al final del diario y del semanal. **Formato post-fase 6: `[capas] sin_clasificar=N capa_null=N c1=N sin_chunks=N`**. Exit 1 si `capa_null>0` (G3) o trigger stale. El `diff` desapareció: sin eco no hay dos copias que comparar. `sin_chunks` en supabase-py: ids de `v_contratos_estado` + `tdr_*` de `contratos` (la vista no tiene esas columnas) | **MEDIDA** 10 sep, run 34437994154: `[capas] sin_clasificar=74004 capa_null=0 c1=54 sin_chunks=0`, `backend=supabase-py` | — | `diff=2005` histórico fue paginación sin ORDER BY |
