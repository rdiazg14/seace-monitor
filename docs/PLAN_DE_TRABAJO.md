> **Estado de producto (1 sep 2026):** el asesor #9/#10/#11 con iteraciones **1–11** está en prod (self-routing, funnel KV→PG, Dashboard conversión 30d). Clasificación IT = keywords en ingesta + Gemini **manual** (`clasificar_gemini.py` no está en el cron). Punto de entrada: [TRASPASO_MAESTRO_SEACE.md](./TRASPASO_MAESTRO_SEACE.md). Foto iter. 1–11: [ESTADO_CIERRE_2026-08-29.md](./ESTADO_CIERRE_2026-08-29.md). Changelog: [CHANGELOG_ITERACIONES.md](./CHANGELOG_ITERACIONES.md). Este PLAN sigue siendo el de **retrieval v2 + pipeline**; varias filas (reranker v2-m3, chunks 200–400, costo $0/mes, checklist Gemini, «sin caché de chat», clasificador híbrido de `/cotizar`) están **desactualizadas**. Si código y PLAN divergen, gana el código.
